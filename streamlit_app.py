...
# Google Drive 파일 ID
file_id = '1N_39zMG1Nf01mpICXja4TBIJUGl1-rJX'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/6nzKR1v/image.jpg/150?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=nf3HquKEY6Y", start_time=0)
